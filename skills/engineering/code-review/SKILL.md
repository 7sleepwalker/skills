...
4. **Pre-flight:** check that the base ref resolves (`git rev-parse <base>`) and the diff is **not empty**. If either fails, stop and say which — do not start the passes with no diff to review. Then measure the diff size (`gh pr diff <n> --stat` for a PR, the matching `git diff --stat` for a local scope). If it is large (rough limit: about 1500+ changed lines or 40+ files), **do not stop**. For each pass, sort the changed files by how many lines changed, review the files with the most changes first, and list the rest as "not reviewed in depth (`<n>` files)". Say this in the output (Step 4), so the reader knows some files got only a light pass.

Do NOT read the changed files yourself — hand each changed file to the pass that reviews it, so the passes stay independent (and can run as parallel workers).

## Step 2 — Run the four review passes

Read [smells.md](smells.md) and build the Step 0 rule digest first. You build each pass's brief and paste the shared text in — so the rule files are read once here, not re-read for every pass.

Run four passes — Q1, Q2, R, S. If your agent can spawn parallel workers (subagents), launch them at once, one worker per pass; otherwise run them yourself in sequence, Q1 → Q2 → R → S. Same rules and same output either way. Give **Q1, Q2, and R** the changed-files list and the base/head diff command — they judge the code, so they read the full diff. Give **S** only the changed-files list, the `--stat` summary, and the PR/ticket text: it checks scope, not code, so it does not need the full diff. Paste the rule digest only where it is used: **Q1** gets all of it; **Q2** gets the parts about the repo's own utilities and test rules; **R** and **S** do not need it — R judges logic, S reads the ticket and PR text. Each pass judges only what this repo requires and cites the `path:line` from the digest. Trust the digest, not memory. Each returns findings as `{severity, file:line(s), impact + cited rule path:line (or "no repo rule; universal principle"), suggested fix, confidence 0-100}`.
...
- **Confidence bar:** for each finding, judge how sure you are it is real, in scope, new (not pre-existing), and not intentional. Give it a score from 0 to 100. Treat the score as a rough level, not an exact number — 75 versus 85 is a judgment call. **Drop anything below 80.** The scope check and `❓ [q]` do not need a score.
...
- **Two axes, never merged.** The ranked findings (Q1/Q2/R) and the Scope check (S) stay separate — never rank one against the other, never let a clean scope check soften a 🔴 or vice versa. A PR can satisfy one axis and fail the other.
