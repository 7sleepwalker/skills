...
4. **Pre-flight:** check that the base ref resolves (`git rev-parse <base>`) and the diff is **not empty**. If either fails, stop and say which — do not start the agents with no diff to review. Then measure the diff size (`gh pr diff <n> --stat` for a PR, the matching `git diff --stat` for a local scope). If it is large (rough limit: about 1500+ changed lines or 40+ files), **do not stop**. Tell each agent to sort the changed files by how many lines changed, review the files with the most changes first, and list the rest as "not reviewed in depth (`<n>` files)". Say this in the output (Step 4), so the reader knows some files got only a light pass.
