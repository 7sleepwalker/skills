...
- **Name capabilities, not tools.** Write "search the codebase", "spawn parallel workers", or "apply the *<name>* method" — not `Grep`, the `Agent` tool, or a `Skill` call. Mark any harness-specific fast path as an optional layer over a portable default, so a skill runs on other agents, not only Claude Code. The craft — the capability vocabulary and the layering rule — is in `boo:writing-for-agents`; the decision is [adr/0009-capability-language.md](./adr/0009-capability-language.md).
