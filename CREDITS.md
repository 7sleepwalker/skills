...
| [code-review](./skills/engineering/code-review/SKILL.md) | `skills/engineering/code-review` — the Fowler smell baseline in [smells.md](./skills/engineering/code-review/smells.md), and the rule that review axes are never merged or reranked |
...
| Mine | Upstream |
| --- | --- |
| [code-review](./skills/engineering/code-review/SKILL.md) | `skills/caveman-review` — the severity prefixes, the `q` category that replaces hedging, and the terse finding style with its carve-outs for security and architecture |
...
| Mine | Upstream |
| --- | --- |
| [code-review](./skills/engineering/code-review/SKILL.md) | `differential-review` — blast radius (weigh a finding by how many callers the changed symbol reaches) and using `git blame` to spot regressions in previously-fixed lines |
